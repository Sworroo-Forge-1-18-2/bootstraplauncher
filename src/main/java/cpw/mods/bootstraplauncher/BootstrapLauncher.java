...
public class BootstrapLauncher {
    ...
    public static void main(String[] args) {
        try {
            ...
            u.putObject(u.staticFieldBase(field), u.staticFieldOffset(field), null);
            field = FileSystemProvider.class.getDeclaredField("loadingProviders");
            u.putBoolean(u.staticFieldBase(field), u.staticFieldOffset(field), false);
        }catch (Exception e){
        ...
        for (Package definedPackage : Thread.currentThread().getContextClassLoader().getDefinedPackages()) {
        ...
        try {
            ...
            Method export = Module.class.getDeclaredMethod("implAddReadsAllUnnamed");
            METHOD_MODIFIERS.invokeExact(export, Modifier.PUBLIC);
            for (Module module : layer.layer().modules()) {
...
//            CPTest.list(myCL).stream().filter(pckg -> pckg.toString().contains("ru.cristalix") || pckg.toString().contains("com.mojang")).forEach(packge -> {
//                try {
//                    moduleClassLoader.loadClass(packge.toString());
...
//        }
        ...
        for (Package definedPackage : moduleClassLoader.getDefinedPackages()) {
            System.out.println(definedPackage);
        }
